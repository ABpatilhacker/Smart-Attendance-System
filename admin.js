...
function loadDashboard() {
  ...
  db.ref("users").on("value", snap => {
    ...
    snap.forEach(u => { if (u.val().approved) { if (u.val().role==="teacher") teachers++; if(u.val().role==="student") students++; } });
    ...
    snap.forEach(u=>{
      ...
      if(!data.approved) {
        ...
        li.innerHTML=`
          ...
          <span class="badge">${data.role.toUpperCase()}</span><br><br>
          <button class="approve-btn primary" onclick="approveUser('${uid}')">✅ Approve</button>
...
function rejectUser(uid){ if(!confirm("Are you sure to reject this user?")) return; db.ref("users/"+uid).remove().then(()=>toast("User rejected ❌")); }
  ...
  db.ref("classes").on("value", snap=>{
    ...
    snap.forEach(c=>{
      ...
      li.innerHTML=`
        ...
        <button class="primary" onclick="openClassDetails('${c.key}')">View Details</button>
      `;
      list.appendChild(li);
      if(select){
...
function addClass(){
  ...
  db.ref("classes/"+id).set({name, subjects:{}, students:{}}).then(()=>{
    ...
    snap.forEach(u=>{
      ...
      if(data.role==="teacher" && data.approved){
        ...
        li.innerHTML=`
          ...
          <button class="primary" onclick="openTeacherProfile('${u.key}')">View Profile</button>
        `;
        list.appendChild(li);
      }
    });
...
function addTeacher(){
  ...
  db.ref("users/"+uid).set({name,email,role:"teacher",approved:true,assignments:{}}).then(()=>{
  ...
  db.ref("users").on("value", snap=>{
    ...
    snap.forEach(u=>{
      ...
      if(data.role==="student" && data.approved){
        ...
        li.innerHTML=`
          <strong>${data.name}</strong> (Roll ${data.roll}) – ${data.email}
          <button class="primary" onclick="openStudentProfile('${u.key}')">View Profile</button>
        `;
        list.appendChild(li);
      }
    });
...
function addStudent(){
  ...
  const uid=db.ref("users").push().key;
  db.ref("users/"+uid).set({name,roll:Number(roll),email,classId,role:"student",approved:true}).then(()=>{
...
function saveSettings(){
  ...
  db.ref("settings").update({minAttendance:Number(val)}).then(()=>toast("Settings saved ✅"));
}
  ...
  db.ref("users/"+uid).once("value").then(snap=>{
    ...
    db.ref("classes").once("value").then(csnap=>{
      csnap.forEach(c=>{
        const cl=c.val();
        if(cl.subjects){
          for(let s in cl.subjects){
            if(cl.subjects[s].teacherId===uid) subjects.push(`${cl.subjects[s].name} (${cl.name})`);
          }
        }
      });
      panel.innerHTML=`
        ...
        <button class="primary" onclick="closePanel('teacherProfile')">Close</button>
      `;
  ...
  db.ref("users/"+uid).once("value").then(snap=>{
    ...
    if(s.classId){
      db.ref("classes/"+s.classId).once("value").then(csnap=>{
        if(csnap.exists()) className=csnap.val().name;
        panel.innerHTML=`
          <h3>${s.name} – Student Profile</h3>
          <p><strong>Email:</strong> ${s.email}</p>
          <p><strong>Roll No:</strong> ${s.roll}</p>
          <p><strong>Class:</strong> ${className}</p>
          <button class="primary" onclick="closePanel('studentProfile')">Close</button>
        `;
        panel.classList.add("active-panel");
      });
    }
  ...
  db.ref("classes/"+classId).once("value").then(snap=>{
    ...
    if(c.subjects){
      for(let s in c.subjects){
        subjectList+=`<li>${c.subjects[s].name} – Teacher: ${c.subjects[s].teacherId}</li>`;
      }
    }
    panel.innerHTML=`
      ...
      <button class="primary" onclick="viewClassStudents('${classId}')">View Students</button>
      <button class="primary" onclick="closePanel('classPanel')">Close</button>
    `;
    panel.classList.add("active-panel");
  });
}
...
function viewClassStudents(classId){
  db.ref("users").once("value").then(snap=>{
    ...
    snap.forEach(u=>{
      const s=u.val();
      if(s.role==="student" && s.classId===classId) students.push(s);
    });
    const panel=document.getElementById("classPanel");
    let listHTML="";
    students.forEach(s=>{
      listHTML+=`<li>${s.name} (Roll ${s.roll})</li>`;
    });
    panel.innerHTML+=`
      <h4>Students</h4>
      <ul>${listHTML||"<li>No students</li>"}</ul>
    `;
  });
}

function closePanel(id){ document.getElementById(id).classList.remove("active-panel"); }
