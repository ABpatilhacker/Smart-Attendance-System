...
auth.onAuthStateChanged(user => {
  ...
  db.ref(`users/${currentTeacher}/name`).once("value").then(snap=>{
    document.getElementById("teacher-name").textContent = snap.val();
  });
...
function showClasses() {
  ...
  view.innerHTML = `<h2>My Classes</h2><div id="class-list" class="card-grid"></div>`;
...
function showClassDetail(classId) {
  closeSidebar();
  const view = document.getElementById("main-view");
  view.innerHTML = `<h2>Class Dashboard</h2>
...
function showDefaulters() {
  closeSidebar();
  const view = document.getElementById("main-view");
  view.innerHTML = `<h2>Defaulters</h2><div id="defaulters-list" class="card-grid"></div>`;
